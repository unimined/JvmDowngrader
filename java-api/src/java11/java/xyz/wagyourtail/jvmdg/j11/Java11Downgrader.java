                                ...
                                if (field.name.equals(fieldInsn.name)) {
                                    ...
                                    switch (insn.getOpcode()) {
                                        case Opcodes.GETSTATIC:
                                            method.instructions.set(insn, new MethodInsnNode(Opcodes.INVOKESTATIC, fieldInsn.owner, "jvmdowngrader$nest$get$" + fieldInsn.name, "()" + fieldInsn.desc, false));
                                            break;
                                        case Opcodes.PUTSTATIC:
                                            method.instructions.set(insn, new MethodInsnNode(Opcodes.INVOKESTATIC, fieldInsn.owner, "jvmdowngrader$nest$set$" + fieldInsn.name, "(" + fieldInsn.desc + ")V", false));
                                            break;
                                        case Opcodes.GETFIELD:
                                            method.instructions.set(insn, new MethodInsnNode(Opcodes.INVOKEVIRTUAL, fieldInsn.owner, "jvmdowngrader$nest$get$" + fieldInsn.name, "()" + fieldInsn.desc, false));
                                            break;
                                        case Opcodes.PUTFIELD:
                                            method.instructions.set(insn, new MethodInsnNode(Opcodes.INVOKEVIRTUAL, fieldInsn.owner, "jvmdowngrader$nest$set$" + fieldInsn.name, "(" + fieldInsn.desc + ")V", false));
                                            break;
                                        default:
                            ...
                            if (methodNode.name.equals(methodInsn.name) && methodNode.desc.equals(methodInsn.desc)) {
                                ...
                                switch (insn.getOpcode()) {
                                    case Opcodes.INVOKESTATIC:
                                        method.instructions.set(insn, new MethodInsnNode(Opcodes.INVOKESTATIC, methodInsn.owner, "jvmdowngrader$nest$" + methodInsn.name, methodInsn.desc, false));
                                        break;
                                    case Opcodes.INVOKEVIRTUAL:
                                        method.instructions.set(insn, new MethodInsnNode(Opcodes.INVOKEVIRTUAL, methodInsn.owner, "jvmdowngrader$nest$" + methodInsn.name, methodInsn.desc, false));
                                        break;
                                    case Opcodes.INVOKESPECIAL:
                                        method.instructions.set(insn, new MethodInsnNode(Opcodes.INVOKESPECIAL, methodInsn.owner, "jvmdowngrader$nest$" + methodInsn.name, methodInsn.desc, false));
                                        break;
                                    default:
                                        throw new RuntimeException("Unexpected opcode: " + insn.getOpcode());
                                }
                            ...
                            if (methodNode.name.equals(methodInsn.name) && methodNode.desc.equals(methodInsn.desc)) {
                                ...
                                switch (methodInsn.bsm.getTag()) {
                                    case Opcodes.H_INVOKESTATIC:
                                        methodInsn.bsm = new Handle(Opcodes.H_INVOKESTATIC, methodInsn.bsm.getOwner(), "jvmdowngrader$nest$" + methodInsn.bsm.getName(), methodInsn.bsm.getDesc(), false);
                                        break;
                                    case Opcodes.H_INVOKEVIRTUAL:
                                        methodInsn.bsm = new Handle(Opcodes.H_INVOKEVIRTUAL, methodInsn.bsm.getOwner(), "jvmdowngrader$nest$" + methodInsn.bsm.getName(), methodInsn.bsm.getDesc(), false);
                                        break;
                                    case Opcodes.H_INVOKESPECIAL:
                                        methodInsn.bsm = new Handle(Opcodes.H_INVOKESPECIAL, methodInsn.bsm.getOwner(), "jvmdowngrader$nest$" + methodInsn.bsm.getName(), methodInsn.bsm.getDesc(), false);
                                        break;
                                    default:
                                        throw new RuntimeException("Unexpected opcode: " + insn.getOpcode());
                                }
                                    ...
                                    for (MethodNode methodNode : target.methods) {
                                        if (methodNode.name.equals(handle.getName()) && methodNode.desc.equals(handle.getDesc())) {
                                            ...
                                            switch (handle.getTag()) {
                                                case Opcodes.H_INVOKESTATIC:
                                                    methodInsn.bsmArgs[j] = new Handle(Opcodes.H_INVOKESTATIC, handle.getOwner(), "jvmdowngrader$nest$" + handle.getName(), handle.getDesc(), false);
                                                    break;
                                                case Opcodes.H_INVOKEVIRTUAL:
                                                    methodInsn.bsmArgs[j] = new Handle(Opcodes.H_INVOKEVIRTUAL, handle.getOwner(), "jvmdowngrader$nest$" + handle.getName(), handle.getDesc(), false);
                                                    break;
                                                case Opcodes.H_INVOKESPECIAL:
                                                    methodInsn.bsmArgs[j] = new Handle(Opcodes.H_INVOKESPECIAL, handle.getOwner(), "jvmdowngrader$nest$" + handle.getName(), handle.getDesc(), false);
                                                    break;
                                                default:
                                                    throw new RuntimeException("Unexpected opcode: " + insn.getOpcode());
                                            }
                                        ...
                                        if (fieldNode.name.equals(handle.getName()) && fieldNode.desc.equals(handle.getDesc())) {
                                            ...
                                            switch (handle.getTag()) {
                                                case Opcodes.H_GETFIELD:
                                                    methodInsn.bsmArgs[j] = new Handle(Opcodes.H_INVOKEVIRTUAL, handle.getOwner(), "jvmdowngrader$nest$get$" + handle.getName(), handle.getDesc(), false);
                                                    break;
                                                case Opcodes.H_PUTFIELD:
                                                    methodInsn.bsmArgs[j] = new Handle(Opcodes.H_INVOKEVIRTUAL, handle.getOwner(), "jvmdowngrader$nest$set$" + handle.getName(), handle.getDesc(), false);
                                                    break;
                                                case Opcodes.H_GETSTATIC:
                                                    methodInsn.bsmArgs[j] = new Handle(Opcodes.H_INVOKESTATIC, handle.getOwner(), "jvmdowngrader$nest$get$" + handle.getName(), handle.getDesc(), false);
                                                    break;
                                                case Opcodes.H_PUTSTATIC:
                                                    methodInsn.bsmArgs[j] = new Handle(Opcodes.H_INVOKESTATIC, handle.getOwner(), "jvmdowngrader$nest$set$" + handle.getName(), handle.getDesc(), false);
                                                    break;
                                                default:
                                                    throw new RuntimeException("Unexpected opcode: " + insn.getOpcode());
                                            }
        ...
        for (String field : fields.keySet()) {
            if (field.contains("(")) {
                ...
                if (name.equals("<init>")) {
                    ...
                    MethodVisitor mv = clazz.visitMethod(Constants.synthetic(isStatic ? Opcodes.ACC_STATIC : 0), "jvmdowngrader$nest$" + name, desc, null, null);
                    mv.visitCode();
                    if (!isStatic) {
                ...
                MethodVisitor mv = clazz.visitMethod(Constants.synthetic(isStatic ? Opcodes.ACC_STATIC : 0), "jvmdowngrader$nest$get$" + field, "()" + desc, null, null);
                mv.visitCode();
                if (!isStatic) {
                    mv.visitVarInsn(Opcodes.ALOAD, 0);
                }
                ...
                mv = clazz.visitMethod(Constants.synthetic(isStatic ? Opcodes.ACC_STATIC : 0), "jvmdowngrader$nest$set$" + field, "(" + desc + ")V", null, null);
                mv.visitCode();
                if (!isStatic) {
                    mv.visitVarInsn(Opcodes.ALOAD, 0);
                }
